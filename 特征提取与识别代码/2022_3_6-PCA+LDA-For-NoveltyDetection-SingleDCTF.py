...
#FalseTestPath='D:/文档/项目相关资料代码/4GLTE/LTE-RACH-FROM-WZN/final/phone9-12-9loc5'
...
for filename in FalseTestFileNames1:
    ...
    image = mimg.imread(FalseTestPath1+ switch + '/' + filename)
    imageresize = image.reshape(image.size, 1)
    TestMat.append(imageresize)
...
for filename in FalseTestFileNames2:
    ...
    image = mimg.imread(FalseTestPath2+ switch + '/' + filename)
    imageresize = image.reshape(image.size, 1)
    TestMat.append(imageresize)
...
for filename in FalseTestFileNames3:
    ...
    image = mimg.imread(FalseTestPath3+ switch + '/' + filename)
    imageresize = image.reshape(image.size, 1)
    TestMat.append(imageresize)
...
for filename in FalseTestFileNames4:
    ...
    image = mimg.imread(FalseTestPath4+ switch + '/' + filename)
    imageresize = image.reshape(image.size, 1)
    TestMat.append(imageresize)
...
fpr, tpr, thresholds = metrics.roc_curve(trueLabel,SimilarityList)
np.save('fpr-single',fpr)
np.save('tpr-single',tpr)
...
##画P-R曲线
precision, recall, thresholds = metrics.precision_recall_curve(trueLabel, SimilarityList)
np.save('precision-single',precision)
np.save('recall-single',recall)
